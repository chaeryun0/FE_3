...
2. 문은 값, 연산자, 키워드, 명령어, 표현식(값으로 평가, 함수나 key, index를 통한 값의 호출도 표현식) 등으로 구성된다.
3. 공백 병합
...
- 함수별로 엄격모드를 다르게 적용할 수 있으나 혼란을 야기할 수 있다.
...
- 변수 이름 규칙
  - 변수는 숫자로 시작할 수 없다.
  - 띄어쓰기가 안된다.
  - 예약어를 사용할 수 없다.(예약어가 사용 불가능한 것은 아님)
...
- 논리 연산자(&&, ||, !, !!, &, |)
  ...
  ```js
  // 앞에 값이 널이냐를 확인하고 싶은 경우, 단락 회로 평가라고 부름
  result1 = 10 || 100;
  result2 = 0 && 100;
  result3 = null || 100;
  result4 = null && 100;
...
- Number(숫자)
  ...
  - 호출 : 변수명
  - 메서드 :
    - 10.toString()는 안됨, 이유는 소수점 (JavaScript의 parsing때문)
    - (10).toString()와 변수명.toString()은 가능
    - num.toFixed()
  - Number()
...
- String(문자열)
  ...
  - 메서드 :
    ...
    - .lastIndexOf()
    - .includes()
    - .slice()   // 어떤 배열의 begin부터 end까지(end 미포함)에 대한 얕은 복사본을 새로운 배열 객체로 반환 (원본 배열 변경 X)
    - .splice()  // 배열의 기존 요소를 삭제 또는 교체하거나 새 요소를 추가하여 배열의 내용을 변경
    - .split()
    - .substring() // string 객체의 시작 인덱스로 부터 종료 인덱스 전 까지 문자열의 부분 문자열을 반환
    - .substr()    // 문자열에서 특정 위치에서 시작하여 특정 문자 수 만큼의 문자들을 반환 (사라질 예정)
    - .toLowerCase()
    - .toUpperCase()
    - .trim()
    - .replace()
    ...
    '5'.repeat(100).split('').map(parseInt) -> 중간에 NaN이 됨, 2번째 인자가 진수이므로
...
- Boolean(논리값)
  ...
  - 형태 : true, false
  - 호출 : 변수명
  - 어떤 것이 true이고 어떤 것이 false인지 판단할 수 있어야 한다.(truthy, falsy)
...
- Array(배열) : object
  ...
  - 메서드 :
    ...
    - slice - immutable (수정 불가한) // 잘라냄
    - splice - mutable   // 교체
    - reduce - immutable // 누적
    - join              // 합치기
    - indexOf          // index 찾아냄
    - includes        // 포함하는지 true,false 반환
    - find           // 찾아내기 // 주어진 판별 함수를 만족하는 첫 번째 요소의 값을 반환
    - concat        // 합치기
    - every        // 배열 안의 모든 요소가 주어진 판별 함수를 통과하는지 테스트, Boolean 반환
  ...
  Array(100)
    .fill(0)
    .map((value, index) => value + index); // 0부터 100개 배열에 담아 한번에 출력 // [0, 1,.. 99]
  Object.keys(Array(100).fill(0)).map(Number);
  // 위와 동일한 결과 but 1부터 시작할 수 없음
  [...Array(100).keys()];
  // 위와 동일한 결과 .fill(0) 생략 가능 // 다음 값 next만 가리킴
  Array.from({ length: 100 }, (v, i) => i + 1); // [1,2,..100]
  // 배열의 길이를 100으로 주고, index +1 해서 1부터 100까지
  ```
...
- Map : object
  ...
  // 3:30
  // }
  // test.1 // error 키 값이 숫자로 되어있는 경우, 멤버 접근 연산자(.)로 접근할 수 없음
  // test[1] // OK
...
- 함수 표현식과 함수 선언식
  ```js
  let 함수표현식 = function () {}; // 사실은 호이스팅이 되긴 하지만, TDZ에 들어감
...
- 콜백함수
  ...
  - 화살표 함수를 콜백함수로 사용했을 경우의 장단점
    - 장점 : 네이밍을 안해도 된다.
    - 단점 : 다른 곳에서 사용할 수가 없다, 콜백지옥에 빠질 수가 있다.
...
- 기명 함수
  ...
  // ES5에서는 빈 문자열이었는데 ES6에서 name 값을 가지는 것으로 바뀌었음
  let cc = (x, y) => x + y;
  ```
...
- `+` : 앞 문자가 하나 이상임
- `*` : 앞 문자가 0개 이상임
- `{n,m}` : 앞 문자가 `n`개 이상 `m`개 이하. `{0,1}` 은 `?`와 같은 의미
...
- 리터럴은 약속된 기호를 사용해 값을 생성하는 것 <br>
  예를 들어 문자를 생성하기 위해서는 작은 따옴표, 큰 따옴표, 템플릿 리터럴 중 하나를 사용 배열을 생성하려면 대괄호를, 오브젝트를 생성하려면 중괄호를 사용해야 한다.
- new String(), new Array(), new Object()의 형식으로 만들어야 하는 것을 약속된 기호로 만들 수 있게 해준 것
- new Object() 의 리터럴 표현이 {}, new Number(1) 의 리터럴 표현이 1, new String("hello") 의 리터럴 표현이 "hello" 이다.
- new Object()는 생성자 함수를 이용한 것
- 정규표현식 리터럴
...
- fetch
  ...
  - Fetch는 비동기 네트워크 통신을 구현하기 위해 사용하는 Web API
...
// target 요소를 element의 자식으로 위치
element.appendChild(target);

// element의 target 자식 요소를 제거
element.removeChild(target);
```

```js
// parentElement.insertBefore(target, location); target요소를 parentElement의 자식인 location 위치 앞으로 이동한다.
...
console.log(cont.firstElementChild); // 첫번째 자식을 찾음
...
console.log(cont.children); // 모든 직계자식을 찾음
