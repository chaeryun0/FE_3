...
<p>6번 게시물에 오신것을 환영합니다. 여기서 fetch를 통해 백엔드로부터 6번 게시물을 요청해야 한다.</p>
// step 2
https://www.studyin.co.kr/study/6/chat
...
function StudyDetail(props){
  ...
  // 예시
  // fetch(`www.test.abc/${num}`)이런식으로 데이터를 가지고 와서 아래에다가 데이터를 뿌려주면 blog 형식이 완성된다.
...
function Chat(){
  ...
  const location = useLocation() // props가 아닌 useLocation으로 받음
  const params = useParams()
  console.log(location)
  console.log(params)
  ...
  // 호준님이 자주 하시는 방식, 비권장, 왜 이렇게 사용하냐면 url의 주소의 깊이가 깊어졌을 때 www/블라.com/a/b/c/d/e 이렇게 되었을 때 앞이 있는 값이 필요한 경우가 종종 있기 때문
